...
Page({
  ...
  onLoad: function (options) {
    ...
    XY.queryCuurentfName(
      options.deptID,
      function (data) {
  
        if (data.data.success == "ok") {

          if (data.data.current.rows.length ) {
            me.setData({
              deptName: data.data.current.rows[0].fName.value,
    ...
    XY.queryCuurentfName(
      ...
      function (data) {
        ...
        if (data.data.success == "ok") {
          ...
          if (data.data.current.rows.length) {
            me.setData({
              managerName: data.data.current.rows[0].fName.value,
              managerID: options.managerID

            })

            XY.XYLogin({
              "openid": data.data.current.rows[0].fPsnID.value, callback: function (data) {




                me.setData({
                  imgsrc: data.userInfo.avatarUrl.value,
  ...
  addPsn: function () {
    var me = this;
    console.log(me.data.deptID +"me.data.deptIDme.data.deptID")
    XY.addPsn(me.data.managerID, me.data.deptID, app.globalData.userInfo.fID.value, app.globalData.orgInfo.fName.value, function (data) {
