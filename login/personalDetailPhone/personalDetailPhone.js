...
//检查手机号是否合法
    ...
    if (phone.length != 11) {
      ...
      if (getChange) {
        ...
        XY.getYanCode(
        this.data.fCellphone,  function (data) {
        
          that.setData({
            huozheng: data.data.success
          })
        })
  ...
  updataPhoneBtn: function () {
    ...
    if (this.data.fCellphone.length!=11) {//手机号不合法
      //给出提示信息并返回
      var that = this;
      this.setData({
        showTopTips: true,
    ...
    if (yanzheng.length =="") {
      ...
      if (yanzheng == huozheng) {
        ...
        var userInfo = { "fID": app.globalData.openid, "cellphone": this.data.fCellphone }
        XY.saveUserInfo(userInfo, function (e) {
          console.log(e, "yyyy")
          app.globalData.userInfo.cellphone.value = me.data.fCellphone
