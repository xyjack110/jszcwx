...
Page({
  ...
  editSelect: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (currentdate > fExpireDate) {//判断充值期限是否到期

          wx.showModal({
            title: '信息提示',
            content: '您公司的服务费到期了,请联系管理员充值！',
            showCancel: false,
            success: function (res) {
  ...
  */
  ...
  LoadCompanyEqptFiles: function (offset, append) {
    ...
    var me = this
    wx.request({
      url: requestUrl + '/queryCompanyIntoEqptFiles',
      method: "POST",
      data: {
  ...
  exportEqptExcel: function (EqptIDs) {
    ...
    var me = this;
    wx.request({
      url: requestUrl + '/creatIntoEqptExcel',
      method: "POST",
      data: {
  ...
  downLoadFile: function () {
    ...
    wx.downloadFile({
      ...
      url: downloadExcelUrl +app.globalData.orgInfo.fCmpyID.value +"newEqptInto.xls",
      success: function (res) {
