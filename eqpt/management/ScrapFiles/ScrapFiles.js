...
Page({
  ...
  LoadRepairInfoRecord: function (offset, append) {
    ...
    var me = this
    wx.request({
      url: requestUrl + '/queryEqptScrapRecordList',
      method: "POST",
      data: {
        psnFilter:psnFilter,
  ...
  exportEqptExcel: function (EqptIDs) {
    ...
    var me = this;
    wx.request({
      url: requestUrl + '/creatScrapEqptExcel',
      method: "POST",
      data: {
  ...
  downLoadFile: function () {
    ...
    wx.downloadFile({
      ...
      url: downloadExcelUrl +app.globalData.orgInfo.fCmpyID.value +"eqptScrapExcel.xls",
      success: function (res) {
  ...
  EqptUseRecordDetail: function (e) {
    ...
    wx.navigateTo({ url: '/eqpt/mine/EqptScapDetail/EqptScapDetail?fID=' + e.currentTarget.id })
  },
